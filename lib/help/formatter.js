...
HelpFormatter.prototype._formatUsage = function (usage, actions, groups, prefix) {
  ...
  if (usage) {
    ...
    // actionParts - groups and actions parts
    var actionParts = this._formatActionsUsage([].concat(optionals, positionals), groups);
    usage = [].concat([prog], actionParts);
    usage = usage.filter(function (part) {
      return !!part;
    }).join(' ');
    ...
    if ((prefix.length + usage.length) > textWidth) {
      ...
      var optionalParts = this._formatActionsUsage(optionals, groups);
      var positionalParts = this._formatActionsUsage(positionals, groups);
      ...
      var _getLines = function (parts, indent, prefix) {
        ...
        parts.forEach(function (part) {
          if (line.length && lineLength + 1 + part.length > textWidth) {
...
HelpFormatter.prototype._formatActionsUsage = function (actions, groups) {
  // format groups and actions; returns a list of strings
  var action, groupPart, part;
  var parts = [];
  var start = 0;
  var self = this; // lint does not like passing 'this' as forEach context
  function formGroup(group) {
    var end;
    if (action === group._groupActions[0]) {
      end = start + group._groupActions.length;
      if (_.isEqual(actions.slice(start, end), group._groupActions)) {
        // format a group if its actions are a sublist of actions
        groupPart = self._formatGroupUsage(group);
        if (groupPart.length) {
  ...
  while (start < actions.length) {
    ...
    if (groupPart === null) {
      // action that is not in a group
      part = this._formatJustActionsUsage([action]);
      if (part.length) {
...
HelpFormatter.prototype._formatGroupUsage = function (group) {
  // format one group; simple constructed version
  // without after the fact cleanup
  var parts = [];
  group._groupActions.forEach(function (action) {
    var part = this._formatJustActionsUsage([action]);
    if (part.length) {
      // remove [] from actions
  ...
  if (parts.length > 1) {
    // the usual group case
    var text = parts.join(' | ');
    text = group.required ? "(" + text + ")" : "[" + text + "]";
    parts = [text];
  } else if (parts.length === 0) {
    // if the group is empty or suppressed
    parts = [];
  } else {
    // all but one action is suppressed
    parts = group.required ? parts : ["[" + parts[0] + "]"];
  }
  return parts;
};

HelpFormatter.prototype._formatJustActionsUsage = function(actions) {
  // format a list of actions; return list
  var parts = [];
  actions.forEach(function (action) {
    var part, optionString, argsDefault, argsString;
