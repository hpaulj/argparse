...
HelpFormatter.prototype._formatActionsUsage = function (actions, groups) {
  ...
  var start, end, action, groupPart, part;
  var parts = [];
  var i = 0;
  var self = this;
  function formGroup(group) {
      if (group._groupActions && action === group._groupActions[0]) {
...
HelpFormatter.prototype._formatGroupUsage = function (group) {
  // format one group
  var actions = group._groupActions;
  var parts = [];
  ...
  var text = parts.join('');
  // remove () if it contains only one action
  text = text.replace(/^\(([^|]*)\)$/g, '$1');
  parts = text.length ? [text] : [];
  return parts;
};
