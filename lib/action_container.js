...
ActionContainer.prototype.getDefault = function (dest) {
  var result;
  result = (_.isUndefined(this._defaults, dest)) ? null : this._defaults[dest];
  this._actions.forEach(function (action) {
...
ActionContainer.prototype._addAction = function (action) {
  var self = this; // use to 'bind' this in callbacks
  ...
  // we want to edit _hasNegativeNumberOptionals, not reasign it
  action.optionStrings.forEach(function (optionString) {
...
ActionContainer.prototype._addContainerActions = function (container) {
  ...
  var self = this; // 'bind' this for callbacks
  var titleGroupMap = {};
  this._actionGroups.forEach(function (group) {
    if (_.has(titleGroupMap, group.title)) {
      throw new Error(_.str.sprintf('Cannot merge actions - two groups are named "%(group.title)s".', group));
    }
    titleGroupMap[group.title] = group;
    console.log('self group title:', group.title);
  });

  // map each action to its group
  // key is based on the action; value is a group
  var groupMap = {};
  ...
  container._actionGroups.forEach(function (group) {
    ...
    if (!_.has(titleGroupMap, group.title)) {
      titleGroupMap[group.title] = self.addArgumentGroup({
        title: group.title,
        description: group.description
      });
      console.log('self add group:', group.title);
    }

    // map the actions to their new group
    group._groupActions.forEach(function (action) {
      groupMap[actionHash(action)] = titleGroupMap[group.title];
      console.log('mapping parent action: ' + actionHash(action) + ' to group: ' + group.title);
    });
  });
  ...
  container._mutuallyExclusiveGroups.forEach(function (group) {
    ...
    group._groupActions.forEach(function (action) {
      ...
      console.log('mapping action: ' + actionHash(action) + ' to xgroup');
    });
  ...
  container._actions.forEach(function (action) {
    ...
    if (!!groupMap[key]) {
      ...
      console.log('adding action: ' + action.getName() + ' to self group: ' + groupMap[key].title);
    }
    else
    {
      self._addAction(action);
      console.log('add action to self', action.getName());
    }
  });
};
...
ActionContainer.prototype._getOptional = function (args, options) {
  ...
  if (!dest) {
    ...
    // optionStringsLong is a list (possibly empty)
...
ActionContainer.prototype._popActionClass = function (options, defaultValue) {
  // defaultValue = defaultValue || null; // Python: defaultValue=None
  if (_.isUndefined(defaultValue)) {   // or (defaultValue == null)
    defaultValue = null;
  }
