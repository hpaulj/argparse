...
var ArgumentError = require('./argument/error_types').ArgumentError;
var ArgumentTypeError = require('./argument/error_types').ArgumentTypeError;
    ...
    if (action.dest !== $$.SUPPRESS) {
      if (!_.has(namespace, action.dest)) {
        if (action.defaultValue !== $$.SUPPRESS) {
          ...
          // }
          // postpone the call to _getValue, so we don't evaluate the
          // defaultValue if it isn't needed
          // change proposed for Python
  ...
  self._actions.forEach(function (action) {
    if (_.indexOf(seenActions, action) < 0) {
      if (action.required) {
        ...
        **/
        if (_.has(action, 'defaultValue') && namespace[action.dest] !== null &&
            action.defaultValue === namespace[action.dest]) {
          namespace[action.dest] = this._getValue(action, action.defaultValue);
        }
      }
    }
  }, this);
...
ArgumentParser.prototype._getValue = function (action, argString) {
  ...
  try {
    ...
    if (err instanceof ArgumentTypeError) {
      ...
      // TypeError, Error
      msg = _.str.sprintf('Invalid %(type)s value: %(value)s', {
        type: name,
        value: argString
      });
