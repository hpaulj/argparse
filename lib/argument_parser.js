...
var ArgumentParser = module.exports = function ArgumentParser(options) {
  ...
  options.parents.forEach(_.bind(function (parent) {
...
ArgumentParser.prototype._parseKnownArgs = function (argStrings, namespace) {
  ...
  this._mutuallyExclusiveGroups.forEach(mutexGroup) {
    var groupActions = mutexGroup._groupActions;
    // loop on mutexGroup._groupActions {
      conflicts = actionConflicts.setdefault(mutexAction, []);
      conflicts.push(groupActions.slice(0,i);
      // i.e. all but group_actions[i]
      conflicts.push(groupActions.slice(i+1,end);    
    };    
  };
  */
  ...
  function actionHash (action) {
      ...
      return action.dest;  // simplest for now
    };    
  var conflicts, groupActions, i, mutexAction, mutexGroup, _i, _j, _len, _len1, _ref, key;
  this._mutuallyExclusiveGroups.forEach(function (mutexGroup) {
    groupActions = mutexGroup._groupActions;
    for (i = _j = 0; _j < groupActions.length; i = ++_j) {
      mutexAction = groupActions[i];
      key = actionHash(mutexAction); 
      // console.log('mutexAction:', i, key);
      conflicts = actionConflicts[key] = []; // works for only one ex group now
  ...
  /*
  console.log('action conflicts',_.keys(actionConflicts).length);
  _.pairs(actionConflicts).forEach( function (pair) { 
    var result=[];    
    pair[1].forEach( function (action) {
      result.push(action.dest);
    });
  ...
  function takeAction(action, argumentStrings, optionString) {
    ...
    if (argumentValues !== action.default) {
      ...
      if (!!actionConflicts[actionHash(action)]) {
        for (var i = 0; i < actionConflicts[actionHash(action)].length; i++) {
          var actionConflict = actionConflicts[actionHash(action)][i];
          if (seenNonDefaultActions.indexOf(actionConflict) >= 0) {
            //console.log(action.dest)
            //console.log(actionConflict)  
  ...
  self._mutuallyExclusiveGroups.forEach(function (group) {
    if (group.required) {
      group._groupActions.forEach(function (action) {
        if (_.indexOf(seenNonDefaultActions, action) >= 0) {
          actionUsed = true;
          return;
          // what does forEach do with the function return?
        }
      });
...
ArgumentParser.prototype._parseOptional = function (argString) {
  ...
  if (argString.match(this._regexpNegativeNumber) &&
      !_.any(this._hasNegativeNumberOptionals)) {
    return null;
  }
