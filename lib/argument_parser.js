...
ArgumentParser.prototype._parseKnownArgs = function (argStrings, namespace) {
  ...
  */
  // via coffee
  var actionConflicts = {};
  var conflicts, groupActions, i, mutexAction, mutexGroup, _i, _j, _len, _len1, _ref;
  for (_i = 0, _len = this._mutuallyExclusiveGroups.length; _i < _len; _i++) {
    // could replace this iter with forEach
    mutexGroup = this._mutuallyExclusiveGroups[_i];
    groupActions = mutexGroup._groupActions;
    _ref = mutexGroup._groupActions;
    for (i = _j = 0, _len1 = _ref.length; _j < _len1; i = ++_j) {
      mutexAction = _ref[i];
      if (!_.has(actionConflicts, mutexAction)) {
        actionConflicts[mutexAction] = [];
      }
      conflicts = actionConflicts[mutexAction];
  ...
  self._mutuallyExclusiveGroups.forEach(function (group) {
    if (group.required) {
      ...
      if (!actionUsed) {
          var names = 'TODO collect action names';
          var msg = 'one of the arguments ... is required';
          self.error(msg);  
      };
    };
  });
